...
const hasDataChanged = (oldData, newData) => {
  ...
  if (Array.isArray(oldData) && Array.isArray(newData)) {
    ...
    const oldLast =
      oldData[oldData.length - 1]?.id || oldData[oldData.length - 1]?._id;
    const newLast =
      newData[newData.length - 1]?.id || newData[newData.length - 1]?._id;
...
export const cacheData = (key, data, ttl = DEFAULT_TTL) => {
  ...
  try {
    ...
    if (estimatedSize > MAX_STORAGE_SIZE) {
      console.warn(
        `Cache item too large (${(estimatedSize / 1024).toFixed(
          2
        )}KB) for localStorage, using memory-only cache:`,
        key
      );
    ...
    if (error.name === "QuotaExceededError") {
      console.warn(
        "localStorage quota exceeded, using memory-only cache for:",
        key
      );
...
const optimizeForCache = (data) => {
  ...
  // Handle object with articles array (API response format)
  if (
    data &&
    typeof data === "object" &&
    !Array.isArray(data) &&
...
export const fetchWithCache = async (url, options = {}, ttl = DEFAULT_TTL) => {
  ...
  try {
    ...
    const optimizedData =
      url.includes("latest_articles") || url.includes("moreRecent")
    ...
    if (typeof window !== "undefined") {
      Object.keys(localStorage).forEach((storageKey) => {
        ...
        if (
          ...
          storageKey.startsWith("azbytegems")
        ) {
          localStorage.removeItem(storageKey);
        }
      });
...
export const cleanupExpiredCache = () => {
  ...
  try {
    ...
    if (keysToRemove.length === 0 && totalSize > 4 * 1024 * 1024) {
      // 4MB threshold
      const entries = [];
      Object.keys(localStorage).forEach((storageKey) => {
        if (storageKey.includes("cache") || storageKey.startsWith("/api/")) {
